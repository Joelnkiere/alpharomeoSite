...
@$categorie=securite($_POST['categorie']);
@$valider=$_POST['envoyer'];
@$edit=$_GET['edit'];

if (isset($_GET['delete_id'])) {
  $req=$bdd->query("DELETE FROM categorie_pub WHERE id_catPub='".$_GET['delete_id']."'");
  echo "<script>alert('la catégorie supprimée avec succès');</script>
...
if (isset($valider)) {
  if (!empty($categorie)) {
    $inserer=$bdd->query("INSERT INTO categorie_pub (libelle) VALUES('$categorie')");
...
$cat=$bdd->query("SELECT * FROM categorie_pub");
//$edit=$_GET['edit'];
$resultat=$bdd->query("SELECT * FROM Categorie WHERE id_catPub=".$edit."");

/*while ($ligne=$resultat->fetch()) {
  # code...
}*/
if ($edit!="") {
  $modification=$bdd->query("UPDATE categorie SET libelle='$categorie' WHERE id_catPub=".$edit."");
  ...
  <div class="content-wrapper">
    ...
    <section class="content">
      <div class="container-fluid">
        ...
        <!--         <input type="hidden" value="<?//echo $ligne['id_catPub'];?>" class="form-control" hidden> -->
                <div class="card-body">
                  <div class="form-group">
                    <label for="exampleInputEmail1">Categorie</label>
                    <input type="text" class="form-control" name="categorie" value="<?php echo $categorie;?>" id="exampleInputEmail1" placeholder="Entrer la categorie" pattern="^[A-Za-zà-ù-0-9-]+$" required="required" alt="veillez inserer le bon caractere">
                  </div>
                </div>
                <!-- /.card-body -->
